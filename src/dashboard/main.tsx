...
function Dashboard() {
  ...
  async function refresh() {
    ...
    if (accessGranted) {
      ...
      await webExtension.runtime.sendMessage({ type: "REGISTER_DASHBOARD_TAB" });
      applyPendingRuleUrl();
    }
  }
  useEffect(() => {
  ...
  async function addPendingUrlToExistingRule(ruleId: string) {
    ...
    try {
      ...
      setNotice(error instanceof Error ? error.message : "The browser did not provide a valid history URL");
    }
  }
  ...
  function applyPendingRuleUrl() {
    if (pendingRuleUrl) {
      ...
      } catch { setNotice("The browser did not provide a valid history URL"); }
      setPendingRuleUrl("");
  ...
  async function resetProtectedData() {
    if (!await requestConfirmation({
      ...
      detail: "Your browser history will not be deleted.",
  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,#edf7e7,transparent_34%),#f5f7f3]">
    <aside className="fixed inset-y-0 left-0 z-10 hidden w-64 flex-col border-r border-[#e0e6dc] bg-white/90 p-5 backdrop-blur dark:border-[#29394a] dark:bg-[#101b28]/95 lg:flex">
      <div className="mb-8 flex items-center gap-3"><img src="/icons/icon-48.png" alt="" className="h-11 w-11" /><div className="min-w-0"><h1 className="m-0 text-xl font-black">Retentia</h1><div className="mt-1"><span className="pill">v{extensionVersion}</span></div></div></div>
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#edf7e7,transparent_34%),#f5f7f3] p-4 dark:bg-[radial-gradient(circle_at_top_left,#193324,transparent_34%),#0c1420] sm:p-7 lg:ml-64 lg:p-12">
      <div className="mx-auto max-w-6xl">
        <div className="card mb-5 flex items-center gap-3 p-3 lg:hidden"><img src="/icons/icon-48.png" alt="" className="h-9 w-9"/><label className="min-w-0 flex-1"><span className="sr-only">Dashboard section</span><select className="field !py-2" value={view} onChange={(event) => selectView(event.target.value as View)}>{DASHBOARD_VIEWS.map(({ id, label }) => <option value={id} key={`mobile-${id}`}>{label}</option>)}</select></label><span className="pill shrink-0">v{extensionVersion}</span></div>
