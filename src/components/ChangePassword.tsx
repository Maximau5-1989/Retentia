...
export function ChangePassword() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      setError("Retentia could not change the password. Try again.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return <form className="mt-7 border-t border-[#e5eae2] pt-6 dark:border-[#2b3a4b]" onSubmit={submit}>
    ...
    {error && <p className="text-sm font-semibold text-red-600">{error}</p>}{message && <p className="text-sm font-semibold text-green-600">{message}</p>}<button className="btn-secondary mt-3 disabled:cursor-not-allowed disabled:opacity-50" disabled={busy} type="submit">{busy ? "Changing…" : "Change password"}</button>
  </form>;
}
