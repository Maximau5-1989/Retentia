...
export function PasswordModal({ mode, onSuccess, onCancel, onReset }: PasswordModalProps) {
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="password-title">
    <form className="modal-panel" onSubmit={submit}>
      <div className="mb-5 flex items-center gap-3"><img src="/icons/icon-48.png" className="h-12 w-12"/><div><p className="muted m-0 text-xs font-bold uppercase tracking-[.14em]">Retentia security</p><h2 id="password-title" className="m-0 text-xl font-black">{mode === "setup" ? "Create your password" : "Unlock Retentia"}</h2></div></div>
      ...
      {error && <p className="mb-0 mt-3 rounded-lg bg-red-50 p-3 text-sm font-semibold text-red-700">{error}</p>}
