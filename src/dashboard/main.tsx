  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,#edf7e7,transparent_34%),#f5f7f3]">
    ...
    <main className="ml-64 min-h-screen bg-[radial-gradient(circle_at_top_left,#edf7e7,transparent_34%),#f5f7f3] p-8 dark:bg-[radial-gradient(circle_at_top_left,#193324,transparent_34%),#0c1420] lg:p-12">
      <div className="mx-auto max-w-6xl">
        ...
        {view === "categories" && categoryScan && <div className="space-y-5">{categoryScan.buckets.filter(bucket => bucket.domains.length > 0).map(bucket => { const preset=getCategoryPreset(bucket.category); return <section className="card overflow-hidden" key={`domains-${bucket.category ?? 'uncategorized'}`}><div className="flex items-center justify-between border-b border-[#e5eae2] p-5 dark:border-[#2b3a4b]"><div><h3 className="m-0 text-base font-extrabold">Classify websites from {preset?.label ?? 'Uncategorized'}</h3><p className="muted mb-0 mt-1 text-xs">Correct any result here. A custom category—or Keep uncategorized—overrides the bundled database and future automatic scans.</p></div><span className="pill">{bucket.domains.length} domains</span></div><div className="max-h-72 overflow-y-auto">{bucket.domains.map(item => <div className="flex items-center gap-3 border-t border-[#edf0ea] px-5 py-3 first:border-t-0 dark:border-[#29394a]" key={`${item.domain}-${item.suggestedCategory ?? bucket.category ?? 'none'}`}><div className="min-w-0 flex-1"><div className="truncate text-sm font-bold">{item.domain}</div><div className="muted text-xs">{item.urls} URLs · {item.visits} visits{item.overridden ? ' · user classification' : ''}{item.suggestedCategory ? ` · Possible ${getCategoryPreset(item.suggestedCategory)?.label}` : ''}</div></div><select aria-label={`Category for ${item.domain}`} className="field !w-56" value={item.overridden ? bucket.category ?? 'uncategorized' : 'automatic'} onChange={event => void moveDomain(item.domain, parseCategoryChoice(event.target.value))}><option value="automatic">Automatic database</option><option value="uncategorized">Keep uncategorized</option>{CATEGORY_PRESETS.map(option => <option value={option.id} key={option.id}>{option.label}</option>)}</select></div>)}</div></section>})}</div>}
