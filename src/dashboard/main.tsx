...
function Dashboard() {
  ...
  function applyCategory(category?: CategoryId) {
    ...
    setDraft(current => ({ ...current, category, ...(current.kind === "category" && category ? { pattern: category } : {}), ...(preset ? { duration: preset.duration, unit: preset.unit } : {}) }));
  }
  ...
  async function cleanCategory(category: CategoryId) {
    const preset = getCategoryPreset(category)!;
    const existing = rules.find((rule) => rule.kind === "category" && rule.pattern === category);
    const cleanupDescription = existing?.deleteImmediately
  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,#edf7e7,transparent_34%),#f5f7f3]">
    ...
    <main className="ml-64 min-h-screen bg-[radial-gradient(circle_at_top_left,#edf7e7,transparent_34%),#f5f7f3] p-8 dark:bg-[radial-gradient(circle_at_top_left,#193324,transparent_34%),#0c1420] lg:p-12">
      <div className="mx-auto max-w-6xl">
        ...
        {view === "categories" && <section><div className="card mb-6 flex flex-wrap items-center justify-between gap-4 p-6"><div><h3 className="m-0 text-lg font-extrabold">History category scanner</h3><p className="muted mb-0 mt-1 max-w-2xl text-sm">Scans your complete available Chrome history locally and groups known websites into categories. Only aggregate counts stay in this open view; URLs, domains, and scan results are never saved.</p></div><button className="btn-primary" disabled={scanningCategories} onClick={runCategoryScan}>{scanningCategories ? 'Scanning history…' : categoryScan ? 'Scan again' : 'Scan all history'}</button></div>{categoryScan ? <><div className="mb-5 grid gap-4 md:grid-cols-3">{[[categoryScan.scanned,'History URLs scanned'],[categoryScan.categorized,'Categorized'],[categoryScan.uncategorized,'Uncategorized']].map(([value,label]) => <div className="card p-5" key={label}><strong className="text-3xl">{value}</strong><p className="muted mb-0 mt-1 text-sm font-semibold">{label}</p></div>)}</div>{categoryScan.resultLimitReached && <div className="mb-5 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-950 dark:text-amber-100">Chrome returned the one-million-result safety limit. The displayed totals may not include older entries beyond that limit.</div>}<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{categoryScan.buckets.map(bucket => { const preset=getCategoryPreset(bucket.category); return <article className="card p-5" key={bucket.category ?? 'uncategorized'}><div className="flex items-start justify-between gap-3"><div><h3 className="m-0 text-base font-extrabold">{preset?.label ?? 'Uncategorized'}</h3><p className="muted mb-0 mt-1 text-xs">{preset?.description ?? 'No reliable local domain match was found.'}</p></div><span className="pill">{bucket.urls}</span></div><div className="muted mt-4 text-xs font-semibold">{bucket.visits} recorded visits</div>{preset && <div className="mt-3 text-xs font-bold">Suggested retention: {preset.duration} {preset.unit}</div>}</article>})}</div><p className="muted mt-5 text-xs">Scan completed {formatDate(categoryScan.runAt)}. Closing or refreshing this page discards these category totals.</p></> : <div className="card p-12 text-center"><h3>Nothing has been scanned yet</h3><p className="muted mx-auto max-w-xl">Start a local scan to see how much of your history matches Retentia's transparent built-in domain lists. This scan never deletes history.</p></div>}</section>}
